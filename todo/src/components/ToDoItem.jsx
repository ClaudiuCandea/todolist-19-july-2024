...
function TodoItem({ todo, isPublic, isOwner }) {
  ...
  const truncatedDescription = todo.description.substring(
    0,
    ...
    <div className="bg-zinc-300 dark:bg-gray-800 text-black dark:text-gray-100 shadow-lg rounded-lg p-6 mb-4 flex flex-row break-words md:flex-1 h-full">
      ...
      <div className="flex flex-col break-words md:flex-1 h-full">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h3 className="text-xl font-bold text-gray-600 dark:text-gray-300 max-w-[300px]">
              {todo.name}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300 italic text-ellipsis max-w-[300px]">
              {todo.category}
            </p>
          </div>
          {isOwner ? (
